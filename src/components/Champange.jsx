...
const Champange = () => {
    ...
    const fetchCocktails = async () => {
      try {
        ...
        if (response.data && response.data.drinks) {
          ...
          setCocktails([]); // Eğer veri gelmezse boş bir array koy
        }
        setLoading(false);
      } catch (error) {
  ...
  return (
    <Container>
      ...
      <hr />
      <Title style={{ color: "black", fontFamily: 'Playfair Display' }}>Cocktail Menu</Title>
      <hr />
      <MenuContainer>
        {currentItems.length > 0 ? (
          ...
          <LoadingMessage>No cocktails available</LoadingMessage>
        )}
      </MenuContainer>
